...
document.getElementById("analyze").onclick = async () => {
  ...
  if (r.ok && r.data.warnings && r.data.warnings.missing_data_count > 0) {
    ...
    warn.innerHTML = `
      <strong>⚠️ Données manquantes détectées:</strong><br>
  ...
  if (r.ok && r.data.downloads) {
    if (r.data.downloads.cba) {
      ...
      a.textContent = "📊 Télécharger CBA (xlsx)";
    ...
    if (r.data.downloads.pv) {
      ...
      a.textContent = "📝 Télécharger PV Draft (docx)";
      a.className = "download";
      a.target = "_blank";
      dl.appendChild(a);
    }
...
document.getElementById("decide").onclick = async () => {
  ...
  if (r.ok && r.data.pv_with_decision) {
    ...
    a.textContent = "📝 Télécharger PV FINAL (avec décision)";
    a.className = "download";
    a.target = "_blank";
