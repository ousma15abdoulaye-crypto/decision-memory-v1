Voici le fichier final — propre, sans marqueurs, prêt à coller directement.

alembic/versions/015_m_extraction_corrections.py — VERSION FINALE
python
...
def upgrade():
    ...
    # ── Aligner extraction_errors : error_detail→error_message ────────
    op.execute("""
        DO $$
        BEGIN
            IF EXISTS (
                ...
                WHERE table_schema='public'
                  AND table_name='extraction_errors'
            ...
            IF EXISTS (
                ...
                WHERE table_schema='public'
                  AND table_name='extraction_errors'
                  AND column_name='requires_human'
            ) THEN
