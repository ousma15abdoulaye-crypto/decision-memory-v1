...
def upgrade(engine: Optional[Engine] = None) -> None:
    ...
    _execute_sql(
        ...
        COMMENT ON TABLE audits IS 'Append-only audit trail (Constitution V3.3.2 §8)';
    """,
    )
    ...
    _execute_sql(
        ...
        COMMENT ON TABLE market_signals IS 'Append-only market memory (Constitution V3.3.2 §8)';
    """,
    )
    ...
    _execute_sql(
        ...
        COMMENT ON TABLE memory_entries IS 'Append-only memory entries (Constitution V3.3.2 §8)';
    """,
    )
