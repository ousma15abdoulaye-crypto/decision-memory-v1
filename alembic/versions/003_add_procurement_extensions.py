...
def upgrade(engine: Optional[Engine] = None) -> None:
    """Crée les extensions procurement: références, catégories, seuils."""
    bind = _get_bind(engine)
    timestamp = datetime.utcnow().isoformat()
    
    # ============================================
    # 1. PROCUREMENT_REFERENCES (M2D)
    # ============================================
    _execute_sql(bind, """
    ...
    # ============================================
    # 3. PURCHASE_CATEGORIES (M2E - Manuel SCI)
    # ============================================
    _execute_sql(bind, """
        CREATE TABLE IF NOT EXISTS purchase_categories (
            ...
            specific_rules_json JSONB,
            created_at TEXT NOT NULL
        )
    """)
    
    # Seed purchase categories (Manuel SCI pages 34-44)
    _execute_sql(bind, f"""
        INSERT INTO purchase_categories (id, code, label, is_high_risk, requires_expert, specific_rules_json, created_at) VALUES
        ('cat_travel', 'TRAVEL', 'Voyages et hôtels', FALSE, FALSE, '{{"max_procedure": "devis_formel"}}'::jsonb, '{timestamp}'),
        ('cat_property', 'PROPERTY', 'Location immobilière', FALSE, FALSE, '{{"legal_review_required": true}}'::jsonb, '{timestamp}'),
        ('cat_constr', 'CONSTR', 'Construction', TRUE, TRUE, '{{"technical_expert_required": true, "site_visit_required": true}}'::jsonb, '{timestamp}'),
        ('cat_health', 'HEALTH', 'Produits de santé', TRUE, TRUE, '{{"qualified_suppliers_only": true}}'::jsonb, '{timestamp}'),
        ('cat_it_sci', 'IT_SCI', 'IT / Technologie', TRUE, FALSE, '{{"section_889_compliance": true, "it_approval_required": true}}'::jsonb, '{timestamp}'),
        ('cat_labor', 'LABOR', 'Main-d''œuvre externe', FALSE, FALSE, '{{"consultancy_fee_limits": true}}'::jsonb, '{timestamp}'),
        ('cat_cva', 'CVA', 'Espèces et bons (CVA)', FALSE, FALSE, '{{"fsp_panel_required": true}}'::jsonb, '{timestamp}'),
        ('cat_fleet', 'FLEET', 'Flotte et transport', FALSE, FALSE, '{{"fleet_fund_priority": true, "safety_standards": true}}'::jsonb, '{timestamp}'),
        ('cat_insurance', 'INSURANCE', 'Assurance', FALSE, FALSE, '{{"provider": "Marsh/MMB", "no_competition": true}}'::jsonb, '{timestamp}'),
        ('cat_generic', 'GENERIC', 'Achats généraux', FALSE, FALSE, '{{}}'::jsonb, '{timestamp}')
        ON CONFLICT (code) DO NOTHING
    """)
    
    # ============================================
    # 4. PROCUREMENT_THRESHOLDS (M2H)
    # ============================================
    _execute_sql(bind, """
    ...
    _execute_sql(bind, """
        ...
        (procedure_type, min_amount_usd, max_amount_usd, min_suppliers, description_en, description_fr)
        VALUES
    ...
    # Add constraint for procedure_type
    _execute_sql(bind, """
        DO $$ 
        BEGIN
...
def downgrade(engine: Optional[Engine] = None) -> None:
    """Supprime les extensions procurement."""
    bind = _get_bind(engine)
