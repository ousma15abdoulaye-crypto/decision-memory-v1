...
def _insert_direct(db_session) -> dict:
    """Insère une entrée minimale valide directement en SQL via psycopg."""
    # Récupère la connexion DBAPI (psycopg) sous-jacente depuis la Session.
    conn = db_session.connection().connection
    with conn.cursor() as cur:
        cur.execute("SELECT nextval('audit_log_chain_seq_seq')")
        (chain_seq,) = cur.fetchone()
...
class TestTriggerDelete:
    def test_delete_rejete_par_trigger(self, db_session) -> None:
...
class TestTriggerUpdate:
    def test_update_rejete_par_trigger(self, db_session) -> None:
...
class TestTriggerTruncate:
    def test_truncate_rejete_par_trigger(self, db_session) -> None:
...
class TestInsertAutorise:
    def test_insert_autorise(self, db_session) -> None:
