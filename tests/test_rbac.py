...
def create_test_user(username: str, email: str, password: str = "testpass") -> dict:
    ...
    response = client.post(
        "/auth/register",
        json={"email": email, "username": username, "password": password, "full_name": f"Test User {username}"},
    )
...
def test_admin_can_access_all_cases():
    ...
    response = client.get("/api/cases", headers={"Authorization": f"Bearer {admin_token}"})
...
def test_procurement_officer_can_create_case():
    ...
    response = client.post(
        "/api/cases",
        json={"case_type": "DAO", "title": f"Test Case by Officer {unique_id}", "lot": None},
...
def test_ownership_check():
    ...
    get_response = client.get(f"/api/cases/{case_id}", headers={"Authorization": f"Bearer {token1}"})
    assert get_response.status_code == 200
    case_data = get_response.json()
    assert (
        case_data["case"]["owner_id"] == user1.get("id") or case_data["case"]["owner_id"] == 1
...
def test_admin_bypass_ownership():
    ...
    create_response = client.post(
        "/api/cases",
        json={"case_type": "DAO", "title": f"Regular User Case {unique_id}", "lot": None},
