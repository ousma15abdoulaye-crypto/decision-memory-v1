...
class TestHorodatageAutomatique:
    ...
    def test_started_at_null_avant_processing(self, db_transaction):
        job_id = _insert_job(db_transaction, status="pending")
        db_transaction.execute(
            "SELECT started_at FROM extraction_jobs WHERE id = %s", (job_id,)
        )
    ...
    def test_completed_at_set_on_failed(self, db_transaction):
        ...
        db_transaction.execute(
            "SELECT completed_at FROM extraction_jobs WHERE id = %s", (job_id,)
        )
