...
Le `Dockerfile` copie `services/annotation-backend/*`, `src/annotation`, `src/procurement`, `src/db`, `src/core` depuis la **racine du dépôt**. Le contexte Docker doit donc être le repo entier.
...
4. **Orchestrateur + Pass 2A (M13)** : le chargement de `src.annotation.orchestrator` importe `pass_2a` → `M13RegulatoryProfileRepository` → **`psycopg`** et les paquets **`src.db` / `src.core`** (dépendances et `COPY` dans le `Dockerfile` du service). Pour la persistance M13 au runtime, définir sur **ce** service **`DATABASE_URL`** (PostgreSQL) et **`SECRET_KEY`** ou **`JWT_SECRET`** (≥ 32 caractères, `src.core.config`) — souvent les mêmes que l’API si base partagée.
