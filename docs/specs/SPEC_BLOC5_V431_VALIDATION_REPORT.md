...
| `committee_session` cohérent avec matrice / FSM | Alignement spec vs DB | DB : `draft`, **`active`**, `in_deliberation`, `sealed`, `closed`. Spec liste `draft`, `in_deliberation`, `sealed`, `closed`, `no_session`. **Mapping API** : `active` → **`draft`** côté réponses agrégées (`map_committee_session_row`) ; `no_session` si aucune ligne `committee_sessions`. | **Documenté** — mapping API explicité pour éviter toute confusion inter-BLOCs |
