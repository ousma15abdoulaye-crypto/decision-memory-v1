...
║  Dernière mise à jour : 2026-04-12 — **Railway prod (dernier enregistrement MRD)** : head **093** (`093_v51_assessment_history`, apply **091→092→093** 2026-04-09) — **apply prod →098** (chaîne **094–098**) documenté **ADDENDUM 2026-04-12** (**#396** **097** · **#398** **098**) (**GO CTO** / **`DMS_ALLOW_RAILWAY=1`**, **`scripts/apply_railway_migrations_safe.py --apply`**) ; **dépôt `alembic heads`** : **098** (`098_primary_admin_email_owner_mandate`) — **PR #398** MERGÉ **`dc975355`** (JWT **`is_superuser`→`admin`**, refresh relit rôle sur **`db_conn`**, migration **098** + seed **004**, **`VALID_ALEMBIC_HEADS`** **098**) ; **PR #396** MERGÉ **`9770aed9`** (M12 agent + **097** CI-safe) ; **PR #391** merge **`68c00042`** (RAG M12, chaîne incl. **096**) ; **base locale** : **`098`** attendu après **`alembic upgrade head`** (session **2026-04-12**) ; **apply prod 093→098** (migrations **094–098**) **= GO CTO** (**RÈGLE-ANCHOR-06**, post-check `docs/ops/SECURITY_HARDENING.md` + runbook **`docs/ops/rag_ingest_runbook.md`** pour **096** + mandat **098**) ; **2026-04-11** — **sécurité multi-tenant (PR #366 + #367)** : Alembic **094** + **095** ; RLS **FORCE** + `tenant_id` ; **`dms_default_tenant_id()`** ; **`tests/security/test_tenant_isolation.py`** ; **`docs/security_audit_report.md`**, **`docs/ops/SECURITY_HARDENING.md`** ; whitelist **`VALID_ALEMBIC_HEADS`** **094+095+096+097+098** ; **E-101** ; **E-103** ; § **ADDENDUM 2026-04-11 — SÉCURITÉ RLS 094–095** ; § **ADDENDUM 2026-04-12 — PR #396** ; § **ADDENDUM 2026-04-12 — PR #398** ; **contexte historique Phase 1** — mandat **DMS-MIGRATION-PROD-V51-001** (GO AO) : **pré-état réel 080** (pas 079) → **081→090** (10 révisions) dont **`090_v51_extraction_jobs_langfuse_trace`** ; pre-check + post-check : `scripts/railway_migration_precheck_v51_001.py`, `scripts/railway_migration_postcheck_v51_001.py` ; rapport `docs/ops/RAILWAY_MIGRATION_V51_001_REPORT.md` ; backup Railway Dashboard Phase 1 **confirmé AO** : **1,4 Go**, **2026-04-08 15h12** ; **2026-04-09** — PR **#357** mergée (`75a66239`) — **E-99** + § **ADDENDUM 2026-04-09 — PR #357** ; **2026-04-10** — **V5.2** — **E-100** + § **ADDENDUM 2026-04-10 — V5.2 CONFIG** ; **2026-04-10** — **JWT pilote** — **cdbc2752** **`WORKSPACE_ACCESS_JWT_FALLBACK`** — § **ADDENDUM 2026-04-10 — JWT WORKSPACE PILOTE** ; **2026-04-11** — **PR #379** MERGÉ main **00689c1a** — M13 persistance : **`get_latest`**, **`profile_index.m13b`**, **`M13_RETENTION_POLICY`**, **030-m13** ; § **ADDENDUM 2026-04-11 — PR #379 M13** ; **2026-04-11** — **PR #387** MERGÉ main **878f1d39** — **frontend-v51** corridor **ZIP → bundles → pipeline V5** (UI) ; § **ADDENDUM 2026-04-11 — PR #387** ; **2026-04-11** — **PR #390** MERGÉ main **64ca7f89** — **dossier → ZIP navigateur** (**`fflate`**, **`zip-directory.ts`**) + clarification **Pass -1** vs **pipeline V5** ; § **ADDENDUM 2026-04-11 — PR #390** ║
...
║  Addendum 2026-04-11 : **M-V52-A** — preuves **R1** (bridge M14→`criterion_assessments`) + **R3** (`market_delta_pct`) : tests **`tests/integration/test_v52_r1_r3_synthetic.py`**, mandat **`docs/mandates/M-V52-A-R1-R3-PROOF.md`**, runbook **`docs/ops/RUNBOOK_V52_R1_R3_SYNTHETIC_VALIDATION.md`** ; correctif **`src/services/market_signal_lookup.py`** (`set_limit` / `similarity` en **REAL** pour **pg_trgm**) ; **`docs/audit/RUPTURES_V52.md`** R1/R3 → ✅ ; synthèse chaîne **`docs/mandates/M-V52-CHAIN-CLOSEOUT.md`** ║
...
║  PR #379 (2026-04-11) : **MERGÉ main 00689c1a** — squash — M13 : `get_latest`, `profile_index.m13b`, `M13_RETENTION_POLICY.md`, `.cursor/rules/030-m13.mdc` ; § ADDENDUM 2026-04-11 — PR #379 M13 ║
║  PR #387 (2026-04-11) : **MERGÉ main 878f1d39** — squash — **frontend-v51** ingestion + pipeline UI + docs ops + E2E ; § ADDENDUM 2026-04-11 — PR #387 ║
║  PR #390 (2026-04-11) : **MERGÉ main 64ca7f89** — squash — **frontend-v51** dossier→ZIP client (**fflate**) + copy Pass-1 vs pipeline ; § ADDENDUM 2026-04-11 — PR #390 ║
...
## ADDENDUM 2026-04-11 — PR #379 M13 PERSISTENCE (PROFIL RÉGLEMENTAIRE)

### Objet

Mandat **strict** : lecture du dernier profil M13 par dossier ; schéma JSONB persisté cohérent ; politique de cycle de vie documentée ; garde-fous agents (sans élargissement pipeline / exception bloquante sur UNKNOWN).

### Changements (merge **00689c1a**)

- **`M13RegulatoryProfileRepository.get_latest(case_id)`** : dernière ligne **`m13_regulatory_profile_versions`** (**`ORDER BY version DESC LIMIT 1`**), retour **`payload`** ou **`None`** (erreur loggée).
- **`build_m13_regulatory_profile_persist_payload`** : hooks **M13B** dans **`profile_index.m13b`** uniquement (plus de clé **`m13b`** à la racine du payload).
- **`docs/ops/M13_RETENTION_POLICY.md`** : append-only par version, pas de purge implicite, lecture via **`get_latest`**, dégradé acceptable et visible.
- **`.cursor/rules/030-m13.mdc`** : périmètre typique des mandats M13 ; **fail loud = observabilité**, pas crash pipeline par défaut sur **`unknown`**.

### Consommation SQL / outils

- Préférer **`payload->'profile_index'->'m13b'`** pour les hooks M13B ; les requêtes supposant **`payload->'m13b'`** à la racine sont **obsolètes** après ce merge.

---

## ADDENDUM 2026-04-11 — PR #387 FRONTEND V5.1 — INGESTION WORKSPACE (UI)

### Objet

Corridor **workspace-first** exploitable depuis le navigateur : dépôt **ZIP** (Pass -1 async), visibilité **bundles**, lancement **pipeline V5** (`run-pipeline`), sans nouveau endpoint backend (réutilisation routes **`/api/workspaces/{id}/…`** existantes).

### Changements (merge **878f1d39**)

- **`frontend-v51/components/workspace/workspace-ingestion-panel.tsx`** : upload **`POST …/upload-zip`** ; liste **`GET …/bundles`** avec refetch conditionnel ; **`POST …/run-pipeline?force_m14=`** ; invalidations React Query (**`evaluation-frame`**, **`cognitive-state`**, **`workspace`**, **`dashboard`**, **`bundles`**).
- **`frontend-v51/lib/api-client.ts`** : **`fetchAuth`** + **`parseOkJson`** (factorisation) ; **`postMultipart`**, **`postEmpty`**.
...
## ADDENDUM 2026-04-11 — PR #390 FRONTEND V5.1 — DOSSIER → ZIP (CLIENT) + CLARIFICATION UX

### Objet

L’endpoint **`upload-zip`** n’accepte qu’un fichier **`.zip`** (validation serveur). L’UI **#387** ne proposait qu’un sélecteur de fichier **ZIP** : les utilisateurs ne pouvaient pas choisir un **dossier** brut. Ce merge ajoute la **compression dans le navigateur** avant l’envoi multipart, et **sépare clairement** dans l’interface le **Pass -1** (réponse HTTP immédiate, travail **async**) du **pipeline V5** (**requête HTTP synchrone**, risque de **timeout** proxy).

### Changements (merge **64ca7f89**)

- **`frontend-v51/lib/zip-directory.ts`** : **`fileListToSupplierZipFile`**, plafond **`MAX_FOLDER_ZIP_BYTES`** (~**400 Mo** de fichiers sources avant ZIP — tout tient en RAM côté client) ; **`fflate`** `zip` avec chemins **`webkitRelativePath`**.
- **`frontend-v51/components/workspace/workspace-ingestion-panel.tsx`** : bouton **« Choisir un dossier… »** ; input **`webkitdirectory`** + **`multiple`** (masqué) ; état **compression locale** ; aide en **trois points** (contrat ZIP, Pass -1 arrière-plan, pipeline synchrone).
- **`frontend-v51/package.json`** (+ lock) : dépendance **`fflate`**.
- **`docs/ops/FRONTEND_V51_WORKSPACE_INGESTION_API_CONTRACT.md`** : note **UI dossier → ZIP** et limite mémoire côté client.
- Commits additionnels sur la branche **PR #390** (revue **Copilot** / agent) : durcissements **guards** taille / formulations — **sans** nouveau **`alembic/versions/`**.

### Limites produit

- Dossiers très volumineux : préférer **ZIP sur le poste** puis champ **fichier ZIP** ; au-delà du plafond UI, message explicite.

### CI

- Tous les checks **GitHub** requis étaient **SUCCESS** avant merge squash sur **`main`** (incl. **`frontend_v51_e2e`**, **`Frontend — OpenAPI types + tsc + next build`**, gates V5.1 / V5.2).
...
## ADDENDUM 2026-04-11 — PR #391 MERGÉ + APPLY **096** LOCAL + MRD

### Objet

Tracer la vérité opposable après merge squash **PR #391** sur **`main`** (**`68c00042`**) : état **Alembic** dépôt **096** ; application effective sur une **base PostgreSQL locale** (session agent, **`DATABASE_URL`** via **`.env` / `.env.local`** — non reproduit ici).

### Faits

- **Git** : **`main`** inclut **RAG corpus M12** (ingest, **`document_corpus`**, **`AGENT_RAG_ENABLED`**, durcissement enterprise, gate V53 **`handlers.py`**, **096** idempotente CI).
- **Alembic local** : avant apply, révision **`095_tenant_id_default_offers_extractions`** ; après **`alembic upgrade head`** : **`096_dms_embeddings_tenant_rls (head)`** — colonne **`tenant_id`** + contrainte unique + **RLS** sur **`public.dms_embeddings`**.
- **MRD** : **`docs/freeze/MRD_CURRENT_STATE.md`** aligné **dépôt 096** + mention apply local ; **Railway prod** inchangé dans ce addendum vs **093** au dernier enregistrement — **apply prod = GO CTO**.
...
- **Post-PR #398** : **`alembic heads`** : **`098_primary_admin_email_owner_mandate`** (révision **`097_fix_test_user_role_supply_chain`** ← **098**).

---

## ADDENDUM 2026-04-12 — PR #398 MERGÉ (JWT SUPERUSER + ALEMBIC **098**)

### Git

- **PR #398** : https://github.com/ousma15abdoulaye-crypto/decision-memory-v1/pull/398 — état **MERGED** **2026-04-12**.
- **Merge commit `main`** : **`dc9753555fa7b57645b803ab7d5ea2bd3c440667`** (squash).

### Objet

Aligner **JWT** et **workspace bypass** : `jwt_role_for_user_row()` ; rotation **refresh** relit le rôle depuis **`users`** JOIN **`roles`** via la connexion **`db_conn`** déjà passée à `rotate_refresh_token` ; migration **098** (`098_primary_admin_email_owner_mandate`) + seed **004** (email admin principal) ; extension **`VALID_ALEMBIC_HEADS`** dans **`tests/test_046b_imc_map_fix.py`**.

### Alembic **098** (`098_primary_admin_email_owner_mandate`)

- Données : promotion / fusion compte admin principal (email propriétaire) + **`is_superuser`** / rôle legacy **`admin`** selon cas (seed `username=admin` vs ligne email existante).
- **Apply** : `alembic upgrade head` — post-check `SELECT version_num FROM alembic_version;` attendu **`098_primary_admin_email_owner_mandate`** après **097** (ou depuis état antérieur selon base).

### Railway (**RÈGLE-ANCHOR-06**)

- Apply **097→098** (et chaîne amont si nécessaire) uniquement sous **GO CTO** + `DMS_ALLOW_RAILWAY=1` + script gouverné (`apply_railway_migrations_safe.py`).

---

## ADDENDUM 2026-04-11 — DETTE FE↔BE V51 (SCellement PATCH + MATRICE CANON + LOGIN /api/auth)

### Décision produit / technique

- **ADR** **`docs/adr/ADR-V51-WORKSPACE-SEAL-VS-COMMITTEE-PV.md`** : **Option A** — `PATCH /api/workspaces/{id}/status` avec `status=sealed` **orchestre** le scellement IRR (`committee_sessions`, snapshot PV, `seal_hash`) via **`src/services/workspace_irr_seal_service.py`** (création session + sync membres si besoin ; réconciliation si session déjà scellée).
- **Matrice** : **`GET /api/workspaces/{id}/comparative-matrix`** — source serveur **`m16`** si ligne(s) dans **`criterion_assessments`**, sinon **`m14`** (payload enrichi comme l’evaluation-frame + **`schema_version`**).
- **Auth UI** : login Next → **`POST /api/auth/login`** ; **`POST /auth/login`** documenté **legacy** dans **`src/auth_router.py`** ; émission JWT : **`resolve_tenant_uuid_for_jwt`** aligné avec **`api_auth_router`**.
- **Front** : `comparative-table` consomme **`comparative-matrix`** ; bannières source / fournisseurs partiels ; **`workspace-events-bridge`** signale le repli ws-token ; PDF drilldown derrière **`NEXT_PUBLIC_FEATURE_PDF_DRILLDOWN=1`** ; garde CI **`scripts/check_frontend_api_surface.py`** + **`frontend-v51/types/consumed-paths.ts`**.
- **ADR roadmap M16 UI** : **`docs/adr/ADR-V51-M16-UI-SCOPE-ROADMAP.md`**.

### Docs ops mises à jour
