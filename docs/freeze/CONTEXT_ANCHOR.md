...
║  Addendum 2026-04-09 : PR #357 — `feat/v51-nl-frontend-e2e-ci` : correctifs CI **MQL** — `src/db/async_pool.py` regex `_NAMED_PARAM_RE` avec lookbehind `(?<!:)` pour ne pas traiter les casts PostgreSQL `::text` / `::numeric` comme paramètres `:name` (KeyError dans `execute_mql_query`) ; **E2E** Playwright `comparative-matrix.spec.ts` strict mode (ciblage grille / `columnheader`) ; **RLS 093** : `set_config(..., NULL)` pour GUC UUID (évite `::uuid` sur `''`) ; templates MQL alignés schéma **042** sur la branche ; **aucun nouveau fichier `alembic/versions/`** dans #357 — **pas d’apply Railway** (head prod inchangé **093**) ║
...
║  PR #357 (2026-04-09) : `feat/v51-nl-frontend-e2e-ci` — NL frontend sprint, proxy Next, E2E CI ; correctifs **asyncpg** binds + MQL + RLS test ; **sans migration Alembic** ║
