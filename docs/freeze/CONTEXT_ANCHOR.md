...
║  Dernière mise à jour : 2026-04-08 — PR #345 MERGÉ main f0a8379c (DMS Canon V5.1.0 implémentation) ; dépôt Alembic head **090** ; CI V5.1 **16 tests** + CI Main + gates **verts** ; Railway prod **079** — apply **080→090** en attente GO AO (dry-run : `python -m alembic upgrade head --sql` — **aucune** exécution DB prod sans mandat) ; suite PR #344 (addendum 2026-04-08 ci-dessous) ; voir MRD_CURRENT_STATE ║
