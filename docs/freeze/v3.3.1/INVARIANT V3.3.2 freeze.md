...
Testable automatiquement en CI (tests exécutables, reproductibles).
Bloquante si échec (CI rouge = merge interdit).
Explicable (un test doit indiquer clairement quelle règle est violée).
...
INV-5	CI verte obligatoire : aucune PR mergée si CI rouge, et interdiction de masquer les échecs	Test statique sur .github/workflows/* : pas de `		true, pas de continue-on-error: true` sur étapes critiques, gates présents
