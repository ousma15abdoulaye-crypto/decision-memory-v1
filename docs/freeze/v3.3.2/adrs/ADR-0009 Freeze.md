...
Source unique : public.scoring_configs Interdit : création d'une table scoring_rules (doublon).
...
scoring_configs WHERE profile_code = <family_code>

scoring_configs WHERE profile_code = 'GENERIC'

Fallback code (autorisé uniquement si scoring_configs est vide) :

acceptable = 1.05, eleve = 1.20
Obligation : tracer dans PriceCheckResult.notes : "fallback hardcoded — scoring_configs empty"
...
Le schéma PriceCheckResult ne contient aucun champ décisionnel (rank, winner, recommendation, selected, etc.)
Les réponses des endpoints /price-check/* ne contiennent pas ces clés
