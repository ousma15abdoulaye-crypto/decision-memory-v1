...
|----------|---------|---------------|----------------|------------------|-------------|
| `/api/workspaces/{id}/upload-zip` | POST | `multipart/form-data`, champ fichier nommé **`file`**, extension `.zip` | `{"workspace_id","status":"accepted","message":…}` | **422** si non-zip ; **403** si permission `bundle.upload` absente | HTTP 200 **ne signifie pas** Pass -1 terminé : traitement **async** (ARQ). Rafraîchir bundles / état cognitif. |
| `/api/workspaces/{id}/bundles` | GET | — | `{"workspace_id","bundles":[…]}` | **403** accès workspace | Liste vide = normal tant que Pass -1 n’a pas produit de lignes `supplier_bundles`. |
