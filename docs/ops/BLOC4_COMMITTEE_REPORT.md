...
**Cible API** : déploiement Railway public (URL du type `https://<service>.up.railway.app` — valeur exacte en variable d’environnement / dashboard).  
...
**Note (revue PR)** : les identifiants d’exemple ci-dessous sont **non secrets** mais **périssables** (run daté). Pour reproduction, utiliser la sortie JSON des scripts ou le ticket d’ops ; ne pas traiter ce fichier comme registre d’IDs canoniques.
...
## Identifiants de session (exemple de run — 2026-04-05)

| Champ | Valeur (schéma) |
|--------|-----------------|
| `reference_code` | `PILOT-COMITE-001-<suffixe>` |
| `workspace_id` | `<uuid_workspace>` |
| `session_id` | `<uuid_session>` |
| Comité « chair » | `<user_id>` + RBAC `procurement_director` |
| Membres | 5 `user_id` distincts (rôles supply_chain, secretary, finance, budget_holder, technical) |

RBAC : `INSERT` dans `user_tenant_roles` pour le chair, tenant `sci_mali`, rôle `procurement_director` (permission `committee.manage`). Valeurs réelles : sortie du script mandat ou ticket d’ops.
...
|---|----------------|---------------------|
| 1 | `session_activated` | `<timestamp UTC du run>` |
...
`GET /api/workspaces/<workspace_id>/committee` retourne entre autres `workspace_id`, `session.seal_hash`, `session.sealed_at` — **pas** de `pv_snapshot` dans le JSON API (colonne présente en base mais non exposée par le SELECT du routeur).
...
| Technicien non votant | **OK** | `technical`, `is_voting=false` |
...
**Dernier run prod** (après push branche, **sans** merge `main`) : [`scripts/bloc4_seal_validation_postfix.py`](../../scripts/bloc4_seal_validation_postfix.py) — `reference_code` **`SEAL-TEST-FINAL-<suffixe>`**, `workspace_id` / `session_id` : **voir sortie JSON** du script (non figés ici).
...
Exécuter **en une seule instruction** après vérification de la cible :  
`python scripts/bloc4_apply_workspace_trigger_v2.py` (dry-run, affiche host/db) puis  
`python scripts/bloc4_apply_workspace_trigger_v2.py --apply` (voir [`scripts/bloc4_apply_workspace_trigger_v2.py`](../../scripts/bloc4_apply_workspace_trigger_v2.py)).
