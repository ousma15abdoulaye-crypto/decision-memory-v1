...
for f in migration_files:
    try:
        ...
        migrations.append(
            {"file": f.name, "revision": revision, "down_revision": down_revision}
        )

    except Exception as e:
        print(f"ERROR reading {f.name}: {e}")
        migrations.append({"file": f.name, "revision": "???", "down_revision": "???"})
...
if file_082.exists():
    print(
        "⚠️  WARNING: 082_p32_dao_criteria_scoring_schema.py exists (created by mistake)"
    )
