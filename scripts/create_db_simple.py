    ...
    for user in users:
        for pwd in passwords:
            ...
            try:
                ...
                conn_str = (
                    f"postgresql://{user}:{pwd}@{'localhost'}:5432/postgres"
                    if pwd
                    else f"postgresql://{user}@localhost:5432/postgres"
                )
                ...
                print(
                    f"DATABASE_URL=postgresql+psycopg://{db_user}:{db_password}@localhost:5432/{db_name}"
                )
                print("=" * 60)
