...
def supplier_col_index(
    start_col_index: int, slot_number: int, width_per_slot: int = 1
) -> int:
    if slot_number < 1:
...
def supplier_col_letter(
    start_col_index: int, slot_number: int, width_per_slot: int = 1
) -> str:
    return col_letter_from_index(
        supplier_col_index(start_col_index, slot_number, width_per_slot)
    )
