...
class ScoringEngine:
    ...
    def calculate_scores_for_case(
        self, case_id: str, suppliers: List[SupplierPackage], criteria: List[DAOCriterion]
        ...
        all_category_scores = commercial_scores + capacity_scores + sustainability_scores + essentials_scores
    ...
    def _calculate_commercial_scores(self, suppliers: List[SupplierPackage], profile: dict) -> List[ScoreResult]:
    ...
    def _calculate_capacity_scores(self, suppliers: List[SupplierPackage], profile: dict) -> List[ScoreResult]:
    ...
    def _calculate_sustainability_scores(self, suppliers: List[SupplierPackage], profile: dict) -> List[ScoreResult]:
        ...
        for supplier in suppliers:
            ...
            scores.append(
                ScoreResult(
                    ...
                    calculation_details={"found_keywords": found_keywords, "keywords_count": len(found_keywords)},
                )
            )

        return scores

    def _calculate_essentials_scores(self, suppliers: List[SupplierPackage], profile: dict) -> List[ScoreResult]:
    ...
    def _calculate_total_scores(
        self, suppliers: List[SupplierPackage], category_scores: List[ScoreResult], profile: dict
    ) -> List[ScoreResult]:
        """Calculate weighted total scores."""
        total_scores = []

        # Get weights from profile
        weights = {"commercial": 0.50, "capacity": 0.30, "sustainability": 0.10, "essentials": 0.10}
        ...
        for supplier in suppliers:
            supplier_scores = {
                s.category: s.score_value for s in category_scores if s.supplier_name == supplier.supplier_name
            }

            # Weighted sum
            total = sum(supplier_scores.get(cat, 0.0) * weight for cat, weight in weights.items())
            ...
            total_scores.append(
                ScoreResult(
                    ...
                    calculation_details={"weights": weights, "category_scores": supplier_scores},
                )
            )
    ...
    def _meets_criterion(self, supplier: SupplierPackage, criterion: DAOCriterion) -> bool:
