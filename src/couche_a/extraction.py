...
def extract_dao_criteria_structured(text: str) -> List[DaoCriterion]:
    ...
    if not criteria:
        criteria = [
            DaoCriterion("Général", "Prix", "Prix total de l'offre", 50.0, ordre_affichage=0),
...
# TYPED EXTRACTION
# ------------------------------------------------------------
def extract_dao_criteria_typed(case_id: str, extracted_criteria: List[Dict]) -> List[Dict]:
    """
    ...
    with get_connection() as conn:
        ...
        sust_weight = sum(c["ponderation"] for c in enriched if c["criterion_category"] == "sustainability")
        db_execute(
            conn,
            """
...
def extract_offer_content(case_id: str, artifact_id: str, filepath: str, offer_type: str):
    """
    ...
    time.sleep(2)
    logger.info(f"Extraction offre {offer_type} terminée pour case {case_id}, artifact {artifact_id}")
