...
async def validate_mime_type(file: UploadFile) -> str:
    ...
    if mime not in ALLOWED_MIME_TYPES:
        raise HTTPException(
            400, f"Invalid file type: {mime}. Allowed: PDF, Word, Excel"
        )
...
async def validate_file_size(file: UploadFile) -> int:
    ...
    if size > MAX_UPLOAD_SIZE:
        raise HTTPException(
            413,
            f"File too large: {size} bytes (max {MAX_UPLOAD_SIZE / 1024 / 1024} MB)",
        )
...
def check_case_quota(case_id: str, new_file_size: int):
    ...
    with get_connection() as conn:
        case = db_execute_one(
            conn, "SELECT total_upload_size FROM cases WHERE id = :id", {"id": case_id}
        )
