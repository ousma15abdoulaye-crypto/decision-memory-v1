...
from src.procurement.m12_engine import (
    M12CorpusPipelineError,
    build_m12_output_for_corpus,
)
...
def run_pipeline_v5(
    ...
    *,
    ...
    try:
        m12 = build_m12_output_for_corpus(
            corpus_text=corpus_text,
            document_id=doc_id,
            run_id=uuid.uuid4(),
        )
    except M12CorpusPipelineError as exc:
        out.error = f"m12_passes_failed:{exc.pass_label}:{exc.detail}"[:500]
        out.stopped_at = "step_m12"
        out.duration_seconds = time.perf_counter() - t0
        logger.exception("[PIPELINE-V5] M12 passes: %s", exc)
        return out
    ...
    linking_payload = m12.process_linking.model_dump(mode="json")
    process_linking_data = [
        {
            "document_id": str(o.get("document_id", "")),
            "m12_linking": linking_payload,
        }
        for o in offers
    ...
    inp = M14EvaluationInput(
        ...
        process_linking_data=process_linking_data,
    )

    skip_m14 = False
    if not force_m14:
        # ADR-0012-P0 : skip seulement si le dernier evaluation_document est au moins
        # aussi récent que l'exécution M13 de *ce* run (sinon M14 recalcule la matrice).
