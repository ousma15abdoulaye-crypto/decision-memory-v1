...
from src.procurement.compliance_models_m13 import M13RegulatoryResolutionError
from src.procurement.m12_bootstrap_pipeline_v5 import build_pipeline_v5_minimal_m12
...
def run_pipeline_v5(
    ...
    *,
    ...
    try:
        ...
        out.error = f"m13_regulatory_unresolved:{exc}"[:500]
        out.stopped_at = "step_m13"
        out.duration_seconds = time.perf_counter() - t0
        logger.exception("[PIPELINE-V5] M13 regulatory unresolved: %s", exc)
        return out
    except Exception as exc:
